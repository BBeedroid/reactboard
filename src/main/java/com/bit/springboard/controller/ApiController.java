...
public class ApiController {
    ...
    public ResponseEntity<?> restFulApi(int boardNo, HttpServletResponse response) {
        ...
        try {
            /*if(boardNo == 6) {
                throw new IllegalArgumentException("매개변수가 잘못 되었습니다.");
            }*/
            ...
            //요청 상태코드
            responseDTO.setStatusCode(HttpStatus.OK.value());
            //request status code == 200일 때(정상)
            //요청이 정상적으로 끝나면 응답 데이터와 요청 상태코드를 가지는 객체를 응답으로 보낸다.
            return ResponseEntity.ok().body(responseDTO);
        } catch (Exception e) {
            //요청 상태코드
            responseDTO.setStatusCode(HttpStatus.BAD_REQUEST.value());
            //에러 발생 시 응답 데이터 대신 에러 메시지를 전송한다.
            responseDTO.setErrorMessage(e.getMessage());
            return ResponseEntity.badRequest().body(responseDTO);
        }
    }
